---
name: sdd-converge
description: Assess the codebase against a feature's spec, plan, and tasks and append remaining unbuilt work as new tasks to tasks.md. Use after implementing when you want to converge or close gaps between specs and code.
---

## User Input

The input text the user provided in the conversation can scope the convergence (e.g., a specific module or requirement to focus on).

You **MUST** consider the user input before proceeding (if not empty).

## Goal

Close the gap between what a feature's specification, plan, and tasks call for and what the codebase currently implements. Read `spec.md`, `plan.md`, and `tasks.md` as the **sole source of intent** (with the constitution as governing constraints), assess the current state of the code, determine which requirements, acceptance criteria, plan decisions, and existing tasks are unmet, incomplete, or only partially satisfied, and **append each piece of remaining work as a new, traceable task** at the bottom of `tasks.md` so that `sdd-implement` can complete it. This skill MUST run only after `sdd-implement` has run on the current `tasks.md`, and after `sdd-tasks` has produced a complete `tasks.md`.
...
**APPEND-ONLY, NEVER REWRITE**: The skill's **only** write is appending a new `## Phase N: Convergence` section to `tasks.md`. It MUST NOT:
...
- modify, create, or delete any application code — completing the appended tasks is the job of `sdd-implement`.

When the codebase already satisfies everything, the skill MUST leave `tasks.md` **byte-for-byte unchanged** (no empty Convergence header) and report a clean result.
...
- Read `.specify/feature.json` for `feature_directory`. If absent, infer the most recently created `specs/` feature directory, or ask the user which feature to converge.
- Derive absolute paths:
  - `SPEC` = `FEATURE_DIR/spec.md`
  - `PLAN` = `FEATURE_DIR/plan.md`
  - `TASKS` = `FEATURE_DIR/tasks.md`
  - `CONSTITUTION` = `memory/constitution.md` (if present)
- If `spec.md`, `plan.md`, or `tasks.md` is missing, STOP with a clear, actionable message naming the prerequisite skill to run (`sdd-specify` for a missing spec, `sdd-plan` for a missing plan, `sdd-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend running the `sdd-implement` skill to complete them; note that a follow-up converge run will find fewer or no remaining items.
...
- [ ] Intent inventory built from spec/plan/tasks and constitution.
- [ ] Codebase assessed; findings classified and severity-graded.
- [ ] `tasks.md` left unchanged (converged) OR a single new `## Phase N: Convergence` section appended (tasks_appended).
- [ ] Findings summary and Next Actions reported in-session.

---
_Ported from [GitHub Spec-Kit](https://github.com/github/spec-kit) `templates/commands/converge.md` (CC0/MIT). Licensed under MIT — see repository LICENSE. Logic preserved for DeepSeek Harness skill execution._